  ...
  <div class="wrapper">
    ...
    <div class="calendar">
      <ul class="date__list">
        ...
        <li class="date__item" v-for="(day, index) in dayList" :key="index">
    ...
    <form class="setDate" @submit.prevent="submit">
      <div class="setDate__wrpa">
        <div class="form_tilt">ОТ</div>
        <div class="input__wrap" v-for="(item, key) of fromDate" :key="key">
      ...
      <div class="setDate__wrpa">
        ...
        <div class="input__wrap" v-for="(item, key) of toDate" :key="item">
          <label for="from">{{ key }}</label>
          <input
            :id="key"
            class="input_date"
...
export default {
  ...
  methods: {
    submit() {
      //Задаём промежуток времени для отображения
      const from = new Date(
        this.fromDate.year,
      ...
      //обрабатываем задачи и отображаем
      this.setDayList();
      this.setToDoList();
    },
    setDateExaple() {
      //Задаём даду для данных из задания
    ...
    setToDoList() {
      //Распределяем "задания" на чанки по дням их выполнения
      const mass = this.toDo.map((el) => {
        return { ...el, parent: el };
      });
      this.toDoList = this.dayList.map((eList, inx) => {
        return mass.filter((eDo) => {
    ...
    correctionDateDay(date, type = "start") {
      ...
      if (type === "start")
        return (
          Math.floor((date - this.UTC) / (24 * 60 * 60 * 1000)) *
            (24 * 60 * 60 * 1000) +
          this.UTC
        );
      if (type === "end")
